...
def send_maintenance_email(maintenance, subscribers):
    ...
    subject = f"Pier Cloud Platform Maintenance: {maintenance['title']}"
    ...
    # Formatar descrição com quebras de linha preservadas
    description_formatted = maintenance['description'].replace('\n', '<br>')
    
    # Conteúdo personalizado da manutenção
    maintenance_content = f"""<p style="line-height: 1.8; margin: 0px;">
        <strong style="font-size: 18px;">{maintenance['title']}</strong><br><br>
        {description_formatted}<br><br>
        <strong>Start:</strong> {maintenance['start_utc']}<br>
        <strong>End:</strong> {maintenance['end_utc']}<br><br>
